...
export default function ContactInfoCards() {
  return (
    <StaggerChildren className="flex flex-col gap-4" staggerDelay={0.12}>
      {infoCards.map((card) => {
        const content = (
          <Card
            key={card.title}
          ...
          >
            <div className="flex items-start gap-4">
              <div className="flex-shrink-0 w-11 h-11 rounded-lg bg-soft-teal flex items-center justify-center text-healing-teal group-hover:bg-healing-teal group-hover:text-white transition-colors duration-300">
                {card.icon}
              </div>
              <div>
